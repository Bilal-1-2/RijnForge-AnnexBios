#### **08-09-2025**

Bilal was 2 uren bezig met zoeken en notion te lezen en 1 uur met  taak verdelen 
Mohammed was bezig met 2 uren debriefing  en 1 uur in notion lezen
Rachael was 2 bezig met technische ontwerpen en 1.5 uur notion te lezen 

#### **09-09-2025**

Bilal was  2 uur  bezig met header en bestanden te Regelen en 1.5 uur bezig  logo te goed te maken
Mohammed was 3 uur bezig met debriefing en plan van aanpak maken 
Rachael was 2 uur bezig met technische ontwerpen en 1 uur met footer
armin niet gekomen

#### **10-09-2025**

Bilal heeft header gemaakt en het duurde 4 uren met  branch mergen 
Mohammed heeft debriefing gemaakt en 2 uren bezig met ERD maken
Rachael was  2 uren bezig met  de technische ontwerpen en heeft het afgemaakt en  1.5 uur bezig met footer 
armin niet gekomen

#### **11-09-2025**
Bilal heeft meer style toegevoegd en heeft met content begonnen het duurde 4 uren.
Mohammed heeft  ERD afgemaakt  in 1.5 uur en he is bezig met hero section
rachel was ziek 

armin niet gekomen

#### **12-09-2025**
Bilal was 2 uur  bezig met content.
Mohammed was 1.5 uur bezig met hero section .
rachel was ziek

#### **15-09-2025**
Bilal was 3 uur bezig films in de content  te zetten
muhammed  was afwezig 
rachel was ziek
#### **16-09-2025**
Bilal was 3 uur bezig met styling van film agenda pagina 
Mohammed was 3 uur bezig met hero section
rachel was 3 uur bezig met footer 
#### **17-09-2025**
Bilal was 5 uur bezig met header/content/film agenda pagina stylen en maken.
Mohammed was 5 uur bezig met de hero section af te maken .
rachel was  5 uur bezig met footer af te maken .

#### **18-09-2025**
Bilal was 6 uur  bezig met detail pagina maken en voor het echte werkende detail pagina maken 
Mohammed was 6 uur bezig met hero section af te maken en beginnen met stoelen te maken 
Rachel was 6 uur bezig met kies je ticket te maken en stylen

#### **19-09-2025**
Bilal was 2 uur bezig met detail pagina sterren en img en detail pagina te stylen 
Mohammed was 2 uur bezig met stoelen in bestel pagina te maken .
Rachel was 2 uur bezig met kies je ticket in de bestel pgina te maken.
#### **22-09-2025**
Bilal was 3.5 uur bezig met detail pagina .
Rachel was 3.5 uur bezig met kies je ticket.
Mohammed was 3.5 uur bezig met stoelen te maken.

#### **23-09-2025**
Bilal was 3 uur bezig met detail pagina .
Rachel was 3 uur bezig met formulier te maken.
Mohammed was 3 uur bezig met stoelen te maken.

#### **24-09-2025**
Bilal was 5 uur bezig met bestel pagina .
Rachel was 5 uur bezig met formulier te maken.
Mohammed was 5 uur bezig met database te maken.

#### **25-09-2025**
Bilal was 6 uur bezig met api web te maken .
Rachel was 6 uur bezig met formulier te maken.
Mohammed was 6 uur bezig met ? te maken.
#### **26-09-2025**
Bilal was 3 uur bezig met api web te maken .
Rachel was 3 uur bezig met bedankt pagina te maken.


#### **28/29-09-2025**
Bilal was 16 uur bezig met api web te maken, inclusief verbeteringen aan booking flows, seat selection, responsive design en API-integraties via api-database.php.



### Belangrijke Fasen en Bijdragen:

- **Vroege Planning (08-09 tot 10-09):** Focus op setup, Notion lezen, debriefing, technische ontwerpen, header, logo en ERD. Bilal deed header/logo setup; Mohammed debriefing en ERD; Rachael technische ontwerpen en footer; Armin afwezig.

- **Content en Structuur (11-09 tot 15-09):** Bilal voegde content en films toe; Mohammed voltooide ERD en begon hero section; Rachael ziek/afwezig.

- **Styling en Pagina's (16-09 tot 17-09):** Bilal stylede header/content/film agenda; Mohammed voltooide hero section; Rachael voltooide footer.

- **Detail en Boekingsfuncties (18-09 tot 22-09):** Bilal bouwde detail pagina; Mohammed werkte aan stoelen; Rachael handelde tickets af.

- **Formulieren en Database (23-09 tot 25-09):** Bilal op booking pagina; Rachael op formulieren; Mohammed op database.

- **API en Laatste Aanpassingen (26-09 tot 28/29-09):** Bilal zwaar op API (16 uur over twee dagen); Rachael op bedankt pagina; Mohammed op niet-gespecificeerde taken.

Totaal uren: Bilal ~50+ uur (piek in API/detail); Mohammed ~30+ uur (planning/database); Rachael ~30+ uur (ontwerp/formulieren); Armin minimaal.

## Commit Toewijzing

Gebaseerd op git logs, commits stemmen overeen met logboek activiteiten. Auteurs: Bilal-1-2 (Bilal, 65 commits), Reiko-dot (Rachael, 20 commits), MohammedK (Mohammed, 7 commits). Geen commits van Armin.

### Wat (Bilal) Deed:
- Header, content, film agenda, detail pagina, booking pagina, API, styling, responsive design, merges.
- Belangrijke commits: Header voltooiing, content updates (films toevoegen), detail pagina iteraties, booking pagina refactoring, API integratie, responsive fixes, bedankt pagina, ratings.


### Wat (Mohammed) Deed:
- Stoelen, hero section, debriefing, database-gerelateerde verwijderingen/merges.
### Wat (Rachael) Deed:
-  Footer, kies je ticket, gegevens-betaalwijze, terms, logos, social icons, merges.



